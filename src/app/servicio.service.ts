...
export class ServicioService {
  ...
  constructor() { 
    ...
    this.socket.on('mensaje_chat',(mensaje) =>{
      this.mensajes_chat.push({usuario: mensaje.usuario, texto: mensaje.texto});
    })
  ...
  entrar(nick_name){
    this.socket.emit('nuevo_usuario', nick_name);
  }
  
  enviar_mensaje(user, text){
    let mensaje = {usuario:user, texto:text};
